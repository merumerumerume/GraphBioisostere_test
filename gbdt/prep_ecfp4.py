...
parser.add_argument("target_dir", type=str, default="origin_data_frag/")
...
def convert_records(df, radius=2, n_bits=2048, n_threads=4):
    ...
    unique_smiles = list(set(df['REF-SMILES'].tolist() + df['PRB-SMILES'].tolist()))
...
joblib.dump(records, os.path.join(args.target_dir, "dataset_consistentsmiles-2048.joblib"))
records = convert_records(data, radius=2, n_bits=1024, n_threads=4)
joblib.dump(records, os.path.join(args.target_dir, "dataset_consistentsmiles-1024.joblib"))
